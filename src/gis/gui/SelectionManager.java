...
public class SelectionManager {
	...
	public synchronized boolean clickedOn(GeoMarker marker) {
		switch (numSelected) {
			case 2:
				int index = isSelected(marker);
				if (index >= 0) {
					deselect(index);
					return true;
				}
	 ...
	 */
	private int isSelected(GeoMarker m) {
		for (int i = 0; i < numSelected; ++i) {
			if (selection[i].equals(m)) {
				return i;
			}
		}
		return -1;
	...
	private void deselect(int index) {
		...
		if (index == 0 && numSelected == 2) {//i.e. 
			selection[0] = selection[1];
		}
		--numSelected;
	}
	
	private void select(GeoMarker m) {
		m.selected = true;
	...
	public synchronized GeoMarker[] getSelection() {
		GeoMarker[] markers = new GeoMarker[numSelected];
